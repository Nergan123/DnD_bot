...
class Base_class:
    ...
    def save_state(self):
        print(f'Saving {self.json_name}_data.json')
        state = {}
        ...
        with open(f'{self.json_name}_data.json', 'w') as f:
            json.dump(state, f)

    def load_state(self):
        print(f'Loading {self.json_name}_data.json')
        # s3 = boto3.client('s3')
        # s3_response = s3.get_object(Bucket=self.STATE_BUCKET, Key=self.STATE_REMOTE_FILE_NAME)
        # state_json = s3_response['Body'].read()
        with open(f'{self.json_name}_data.json', 'r') as f:
